...
public class EratosthenesSieve {
    ...
    static HashMap<Long, Boolean> primesToo = new HashMap<>(MAX);
    ...
    public static void main(String[] args) {
        ...
        for (int k = 1; k <= TIMES; k++) {
            primesToo = new HashMap<>(MAX);
            primes = new HashSet<>(PRIME_SEEDS);
            nextUntestedNumber = 2012;
...
//                System.out.println(i);
            ...
            System.out.format("%d primes found %d times in %.3f seconds and %d operations.\n",
                    count/k, k, (double) (end - start) / (double) 1000000000, operations);
...
//                System.out.println(i);
    ...
    public static boolean isPrimeToo(long number) {
        ...
        if (primesToo.containsKey(number)) {
            return primesToo.get(number);
        }

        if (number == 2 || number == 3) {
            primesToo.put(number, true);
            return true;
        }
        if (number % 2 == 0 || number % 3 == 0) {
            primesToo.put(number, false);
            return false;
        }
    ...
    public static void sieve(long number, UlamSpiralController controller) {
        ...
        // Remove known primes
        List<Long> listOfPrimes = new ArrayList<>(primes);
        ...
        for (long aPrime : listOfPrimes) {
            for (long multipleOfCurrentPrime = aPrime * aPrime;
                ...
                operations++;
                if (unchecked.remove(multipleOfCurrentPrime)) {
                    removed = true;
                    if (controller != null) {
            ...
            if (!removed || unchecked.isEmpty()) break;
        }

        while (!unchecked.isEmpty()) {
            long currentPrime = Collections.min(unchecked);
